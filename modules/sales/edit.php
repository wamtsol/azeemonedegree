...
<form action="sales_manage.php?tab=edit" method="post" enctype="multipart/form-data" name="frmAdd" class="form-horizontal form-horizontal-left">
            ...
            <div class="col-sm-10">
                <select name="customer_id" id="" class="margin-btm-5 selectbox">
					...
					if( numrows( $rs ) > 0 ) {
						while( $r = dofetch( $rs ) ) {
							?>
							<option value="<?php echo $r[ "id" ]?>"<?php echo $customer_id==$r["id"]?" selected":""?>><?php echo unslash($r[ "customer_name" ])?></option>
							<?php
						}
					}
					?>
    ...
    <div class="form-group">
                            ...
                            <tr class="sale_item">
                                ...
                                <td>
                                    <select name="items[]" id="items<?php echo $sn?>"  class="item_select">
                                        <option value="">Select Package</option>
                                        <?php
                                        $sql="select * from items where status=1 $adminId order by title";
                                        $rs=doquery($sql, $dblink);
                                        if(numrows($rs)>0){
                                            while($r=dofetch($rs)){
                                                ?>
  	...
  	<div class="form-group">
    	<div class="row">
            ...
            <div class="col-sm-10">
                <input type="submit" value="SAVE AND PRINT" class="btn btn-default btn-l" name="sales_edit" title="Submit Record" />
                <input type="submit" value="SAVE" class="btn btn-success btn-l" name="sales_edit" title="Submit Record" />
            </div>
        </div>
  	</div>
</form>