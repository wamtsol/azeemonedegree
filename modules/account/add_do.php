...
if(isset($_POST["account_add"])){
	...
	if($err==""){
		$sql="INSERT INTO account (admin_id, title, type, description, balance, is_petty_cash) VALUES ('".$adminId."', '".slash($title)."', '".slash($type)."', '".slash($description)."', '".slash($balance)."', '".slash($is_petty_cash)."')";
		doquery($sql,$dblink);
